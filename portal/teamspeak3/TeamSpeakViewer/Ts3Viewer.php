 ...
 * -----------------------------------------------------------------------
 * Began:       2010
 * Date:        $Date: 2010-01-21 05:54:15 +0100 (Do, 21 Jan 2010) $
 * -----------------------------------------------------------------------
 * @author      $Author: Sylna $
 ...
 * @version     $Rev: 7061 $
 * 
 * $Id: ts_func.php 7061 2010-01-21 04:54:15Z Sylna $
 */
...
class Ts3Viewer{
	...
	public function __construct($conf_plus){
		// INIT variables
		...
		$this->info['ts3_cut_names'] = $conf_plus['pk_ts3_cut_names'];
		...
		$this->info['serverinfo']['virtualserver_platform']['show'] = $conf_plus['pk_ts3_stats_showos']; //Show on wich OS TS3 run
		$this->info['serverinfo']['virtualserver_platform']['label'] = 'TS3 OS'; 

		$this->info['serverinfo']['virtualserver_version']['show'] = $conf_plus['pk_ts3_stats_version']; //Show the TS3 server version
		$this->info['serverinfo']['virtualserver_version']['label'] = 'TS3 Version'; 
		
		$this->info['serverinfo']['virtualserver_channelsonline']['show'] = $conf_plus['pk_ts3_stats_numchan']; //Show the number of channels
		$this->info['serverinfo']['virtualserver_channelsonline']['label'] = 'Channnels'; 
		
		$this->info['serverinfo']['virtualserver_uptime']['show'] = $conf_plus['pk_ts3_stats_uptime']; //Show the server uptime since the last restart
		$this->info['serverinfo']['virtualserver_uptime']['label'] = 'Uptime';

		$this->info['serverinfo']['virtualserver_created']['show'] = $conf_plus['pk_ts3_stats_install']; //Show when the server was installed
	...
	public function gethtml(){
		...
		if ($this->noError) {
			...
			$htmlout 	.= '<link rel="stylesheet" type="text/css" href="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/ts3view.css'.'">';	
			...
			$htmlout	.= '<div class="tsca"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/serverimg.png'.'" alt="'.$this->replace($this->sinfo['virtualserver_welcomemessage']).'" title="'.$this->replace($this->sinfo['virtualserver_welcomemessage']).'"/></div>';
	...
	protected function parse_error($msg){
		...
		if (strpos($msg, 'error id=3329') !== false) {
			$this->error[] = 'Queryclient has ban, check flooding settings. Please take a look at the <a href="http://wiki.eqdkp-plus.com/de/index.php/Teamspeak3">wiki</a>';
		}
			...
			foreach ($this->clist as $key => $var) {
				if($var['pid'] == $id){
					if(($this->info['populated_only'] != '1') or ($this->num_clients_recursive($var['cid']) >= '1')){
						...
						$return .= '<div class="tsca"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/channel.png'.'" alt="'.$this->replace($var['channel_topic']).'" title="'.$this->replace($var['channel_topic']).'" /></div>';
						$return .= '<div class="tsca">'.$this->cut_channel($var['channel_name'],$this->info).'</div>';
						if($var['channel_flag_default'] == 1){
							$return .= '<div class="tsca" style="float:right;"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/home.png'.'" alt="'.$this->replace($var['channel_topic']).'" title="'.$this->replace($var['channel_topic']).'" /></div>';
						}

						if($var['channel_flag_password'] == 1){
							$return .= '<div class="tsca" style="float:right;"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/schloss.png'.'" alt="'.$this->replace($var['channel_topic']).'" title="'.$this->replace($var['channel_topic']).'" /></div>';
						}
								...
								if($u_var['cid'] == $var['cid']){
									...
									foreach ($g_temp as $sg_var) {
										if(isset($this->info['sgroup'][$sg_var]['p'])){
											$g_img .= '<div class="tsca" style="float:right"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/'.''.$this->info['sgroup'][$sg_var]['p'].'" alt="'.$this->info['sgroup'][$sg_var]['n'].'" /></div>';
										}
									}
									if(isset($this->info['cgroup'][$u_var['client_channel_group_id']]['p'])){
										if(isset($this->info['cgroup'][$u_var['client_channel_group_id']]['p'])){
											$g_img .= '<div class="tsca" style="float:right"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/'.''.$this->info['cgroup'][$u_var['client_channel_group_id']]['p'].'" alt="'.$this->info['cgroup'][$u_var['client_channel_group_id']]['n'].'" /></div>';
										}
									}
									$return .= $platzhalter.'<div class="tsleer">&nbsp;</div><div class="tsleer">&nbsp;</div><div class="tsca"><img src="'.$eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/'.''.$p_img.'" alt="Player" /></div><div class="tsna">'.$this->cut_names($u_var['client_nickname'],$this->info).'</div>'.$g_img.'<div style="clear:both"></div>';
								}
							}
						}
					}
	...
	protected function replace($var){
		...
		return str_replace($search, $replace, htmlentities($var));
	}
	...
	protected function build_legend(){
		...
		if($this->info['legend'] == '1'){
			...
			foreach ($this->info['sgroup'] as $var) {
				$return .= '<div class="tsle"><img src="' . $eqdkp_root_path . 'portal/teamspeak3/TeamSpeakViewer/tsimages/'.''.$var['p'].'" alt="'.$var['n'].'" /></div>';
