 ...
 * -----------------------------------------------------------------------
 * Began:       2008
 * Date:        $Date: 2011-03-21 17:07:19 +0100 (Mon, 21 Mar 2011) $
 * -----------------------------------------------------------------------
 * @author      $Author: hoofy $
 ...
 * @version     $Rev: 10151 $
 *
 * $Id: rli.class.php 10151 2011-03-21 16:07:19Z hoofy $
 */
...
if(!class_exists('rli'))
{
  class rli
  {
        ...
        foreach ($this->bonus['zone'] as $zo) {
      	...
      	foreach($this->bonus['boss'] as $bo) {
      		if(in_array(trim($zone), $bo['string'])) {
      			...
      			$retu['timebonus'] = $this->bonus['zone'][$bo['tozone']]['bonusph'];
      			$retu['bonus'] = $this->bonus['zone'][$bo['tozone']]['bonus'];
      			break;
      		}
      	}
      }
    ...
    function get_diff_bosskills($bosskills)
    {
    	...
    	foreach($bosskills as $key => $bosskill) {
    		...
    		foreach($this->bonus['boss'] as $boss) {
    			if(count(array_intersect($this->bonus['boss'][$bkey]['string'], $boss['string'])) >= 1 AND (($bosskill['diff'] AND $bosskill['diff'] == $this->bonus['boss']['diff']) OR (!$bosskill['diff'] OR !$this->bonus['boss']['diff']))) {
			...
			foreach($this->bonus['boss'] as $boss)
			{
				...
				if(in_array($bosskill['name'], $boss['string']) AND $bosskill['time'] >= $begin AND $bosskill['time'] <= $end
				   AND (($boss['diff'] AND $bosskill['diff'] == $boss['diff']) OR !$boss['diff'])) {
					...
					$rbosskills[$i]['bonusph'] = $boss['bonusph'];
					$rbosskills[$i]['note'] = $boss['note'];
					$rbosskills[$i]['time'] = $bosskill['time'];
					$rbosskills[$i]['diff'] = $boss['diff'];
					break;
				}
			}
			$i++;
	...
	function get_member_times($member, $begin=false, $end=false)
	{
        ...
        return $this->check_times($times, $begin, $end);
    }

    function check_times($times, $begin=false, $end=false)
    {
    	global $user;
	...
	function calc_timedkp($begin, $end, $player, $timebonus, $boss_time=true, $bosskills=array())
	{
      $timedkp = 0;
	  if($this->config['use_dkp'] & 2) {
		...
		if($boss_time) {
			$timedkp = $timedkp + $this->calc_boss_timedkp($begin, $end, $player, $bosskills);
		}
	  }
	  return $timedkp;
	}

	function calc_boss_timedkp($begin, $end, $player, $bosskills) {
		$timedkp = 0;
		foreach($bosskills as $b => $kill) {
			if($kill['bonusph'] > 0) {
				$temp = $this->get_bosskill_raidtime($begin, $end, $kill['time'], $bosskills[$b-1]['time'], $bosskills[$b+1]['time']);
				$timedkp = $timedkp + $this->calc_timedkp($temp['begin'], $temp['end'], $player, $kill['bonusph'], false);
			}
		}
		return $timedkp;
	}
	...
	function get_raidvalue($begin, $end, $bosskills, $timebonus, $event, $boss_time=false)
	{
		$dkp = 0;
		$tempmem['times'][1]['join'] = $begin;
		$tempmem['times'][2]['leave'] = $end;
		$timedkp = $this->calc_timedkp($begin, $end, $tempmem, $timebonus, $boss_time, $bosskills);
    ...
    function get_adj_raidkeys($raids = false)
    {
    	$raids = ($raids === false) ? $this->data['raids'] : $raids;
		if($this->config['attendence_raid']) {
			$adj_ra['start'] = 0;
			if($this->config['attendence_end']) {
				$adj_ra['end'] = max(array_keys($raids));
			}
		} else {
			$adj_ra['start'] = ($this->config['attendence_begin']) ? 1 : 0;
			$adj_ra['end'] = ($this->config['attendence_end']) ? max(array_keys($raids)) : 0;
		}
		unset($raids);
		return $adj_ra;
	}
      	...
      	if(is_array($raid['bosskills']))
      	{
      	  foreach($raid['bosskills'] as $u => $bk)
      	  {
      		if(!isset($bk['delete']))
      		{
      			...
      			$bosskills[$u]['bonusph'] = floatvalue($bk['bonusph']);
      			$bosskills[$u]['note'] = $bk['note'];
      			$bosskills[$u]['diff'] = $bk['diff'];
      		}
      	  }
      	}
				...
				if($mem['raid_list'])
				{
					...
					foreach($raids as $raid_id)
					{
					  if(($raid_id != $adj_ra['start'] AND $raid_id != $adj_ra['end']) OR !$this->config['attendence_raid'])
					  {
						...
						if($this->config['use_dkp'] & 2) {
							$dkp = $dkp + $this->calc_timedkp($raid['begin'], $raid['end'], $member, $raid['timebonus'], true, $raid['bosskills']);
						}
						if($this->config['use_dkp'] & 1) {
	...
	function parse_plus_string($xml)
	{
		...
		foreach($xml->zones->children() as $zone)
		{
			$this->data['zones'][$key] = array(
				'enter'	=> (int) trim($zone->enter) + $this->config['timezone_offset']*3600,
				'leave'	=> (int) trim($zone->leave) + $this->config['timezone_offset']*3600,
		...
		foreach($xml->bosskills->children() as $bosskill)
		{
			$this->data['bosskills'][$key] = array(
				...
				'time'	=> (int) trim($bosskill->time) + $this->config['timezone_offset']*3600,
		...
		foreach($xml->members->children() as $member)
		{
			...
			foreach($member->times->children() as $time)
			{
				...
				{
					$type = (string) $type[0];
					$time = (int) $time + $this->config['timezone_offset']*3600;
		...
		foreach($xml->items->children() as $item)
		{
			...
			$this->data['loots'][$key] = array(
				...
				'time'		=> (int) trim($item->time) + $this->config['timezone_offset']*3600,
	...
	function parse_eqdkp_string($xml, $ka_rt=false)
	{
		global $user;

		$this->data = array();
		$this->data['zones'][1]['enter'] = strtotime($xml->start) + $this->config['timezone_offset']*3600;
		$this->data['zones'][1]['leave'] = strtotime($xml->end) + $this->config['timezone_offset']*3600;
		$this->data['zones'][1]['name']  = trim($xml->zone);
		$this->data['zones'][1]['diff'] = ($ka_rt) ? trim($xml->instance->difficulty) : trim($xml->difficulty);
		$i = 0;
		foreach ($xml->BossKills->children() as $bosskill)
		{
			$this->data['bosskills'][$i]['name'] = trim(utf8_decode($bosskill->name));
			$this->data['bosskills'][$i]['time'] = strtotime($bosskill->time) + $this->config['timezone_offset']*3600;
			$this->data['bosskills'][$i]['diff'] = ($ka_rt) ? trim($bosskill->instance->difficulty) : trim($bosskill->difficulty);
			$i++;
		}
		$i = 0;
		foreach($xml->Loot->children() as $loot)
		{
			...
			$this->data['loots'][$i]['time']    = strtotime($loot->Time) + $this->config['timezone_offset']*3600;
		...
		foreach ($xml->Join->children() as $joiner)
		{
		    ...
		    if ($key)
	    	{
				$this->data['members'][$key[0]]['jl']['join'][] = strtotime($joiner->time) + $this->config['timezone_offset']*3600;
			}
		}
		foreach ($xml->Leave->children() as $leaver)
		{
		    ...
		    if ($key)
		    {
			   $this->data['members'][$key[0]]['jl']['leave'][] = strtotime($leaver->time) + $this->config['timezone_offset']*3600;
			}
		}
		foreach($this->data['members'] as $key => $member)
		{
	...
	function parse_magicdkp_string($xml)
	{
		...
		foreach ($xml->Join->children() as $joiner)
		{
			$this->data['members'][] = array(
				'jl' => array(
					'join' => array(
						strtotime($joiner->time) + $this->config['timezone_offset']*3600
					)
				),
	...
	function check_ka_rt_format($string) {
		return $this->check_eqdkp_format($string);
	}

	function parse_ka_rt_string($xml) {
		return $this->parse_eqdkp_string($xml, true);
	}
	
	function check_rift_format($string) {
		return $this->check_eq_format($string);
	}
	
	function parse_rift_string($string) {
		$string = $string->Parties;
		$k = 1;
		foreach($string->children() as $party) {
			foreach($party->Members->children() as $member) {
				$this->data['members'][$k]['name'] = (string) $member->Name;
				$this->data['members'][$k]['times'][0]['join'] = 1; //date in the past
				$this->data['members'][$k]['times'][1]['leave'] = mktime();  //since the raid ist definetly over now
				$k++;
			}
	...
	function check_dkplp_format($string) {
		return $this->check_eq_format($string);
	}
	
	function parse_dkplp_string($string) {
		$k = $j = $l = 1;
		foreach($string->dayParticipation->intervalParticipations->children() as $interval) {
			$this->data['zones'][$k]['enter'] = floatval($interval->timeInterval->beginning)/1000;
			$this->data['zones'][$k]['leave'] = floatval($interval->timeInterval->end)/1000;
			$k++;
			foreach($interval->participants->children() as $member) {
				$this->data['members'][$j]['name'] = (string) $member->name;
				$this->data['members'][$j]['times'][0]['join'] = 1; //date in the past
				$this->data['members'][$j]['times'][1]['leave'] = mktime();  //since the raid ist definetly over now
				$j++;
			}
		}
		foreach($string->loot->children() as $item) {
			$this->data['loots'][$l]['name'] = (string) $item->name;
			$this->data['loots'][$l]['time'] = floatval($item->time)/1000;
			$this->data['loots'][$l]['player'] = (string) $item->buyer->member->name;
			$this->data['loots'][$l]['dkp'] = floatvalue((string) $item->price);
			$l++;
		}
	}
